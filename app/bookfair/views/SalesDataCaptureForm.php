...
class SalesDataCaptureForm extends TCPDF {
	...
	public function Render() {
        ...
        foreach($this->_allocations as $row) {
			...
			$this->writeHTMLCell(0, 0, '', '', '<span color="#548DD4"><b>' . $row->name . '</b></span>', 0, 1, false, true, 'L', false);
			$this->setCellPaddings(1, 1, 1, 1);
			$this->setCellMargins(1, 0, 1, 5);
			$this->Cell(23, 8, $row->label, 0, 0, 'C', false, '', 0, false, 'T', 'C');
			$this->Cell(13, 8, floor($row->delivered) == $row->delivered ? number_format($row->delivered,0) : number_format($row->delivered,2), 1, 0, 'C', false, '', 0, false, 'T', 'C');
