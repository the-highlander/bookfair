...
class Statistic extends Eloquent {
    ...
    public function scopeForDivision($query, $bookfair, $division) {
        return $query->where('bookfair_id', $bookfair)
                        ->whereIn('category_id', function ($query) use ($division) {
                            $query->select('id')
                            ->from('categories')
                            ->whereIn('section_id', function($query) use ($division) {
                                $query->select('id')
                                ->from('sections')
    ...
    public function scopeForSection($query, $bookfair, $section) {
        return $query->whereBookfairId($bookfair)
                        ->whereIn('category_id', function ($query) use ($section) {
                            $query->select('id')
                            ->from('categories')
